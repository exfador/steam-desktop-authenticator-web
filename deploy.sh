...
TLS_EMAIL="${TLS_EMAIL:-}"
SETUP_AUTH="${SETUP_AUTH:-yes}"
AUTH_USER="${AUTH_USER:-admin}"
AUTH_PASS="${AUTH_PASS:-}"
COXER_PROXY="${COXER_PROXY:-}"
APP_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" 2>/dev/null && pwd || pwd)"
VAULT_DIR="${VAULT_DIR:-$APP_DIR/maFiles}"
HTPASSWD="/etc/nginx/.coxerhub.htpasswd"
REPO="${REPO:-https://github.com/exfador/steam-desktop-authenticator-web.git}"
BRANCH="${BRANCH:-main}"
INSTALL_DIR="${INSTALL_DIR:-/opt/sda}"
...
if [ ! -f "$APP_DIR/server.js" ]; then
  log "Проект не найден рядом — скачиваю репозиторий в $INSTALL_DIR (ветка $BRANCH)"
  export DEBIAN_FRONTEND=noninteractive
  have git || { apt-get update -qq; apt-get install -y --no-install-recommends git ca-certificates curl; }
  mkdir -p "$(dirname "$INSTALL_DIR")"
  if [ -d "$INSTALL_DIR/.git" ]; then
    info "Папка уже есть — обновляю (git fetch + reset)"
    git -C "$INSTALL_DIR" fetch --depth 1 origin "$BRANCH"
    git -C "$INSTALL_DIR" reset --hard "origin/$BRANCH"
  elif [ -e "$INSTALL_DIR" ] && [ -n "$(ls -A "$INSTALL_DIR" 2>/dev/null)" ]; then
    die "Папка $INSTALL_DIR существует и не пуста (и это не git-репозиторий). Удалите её или задайте INSTALL_DIR=другой/путь."
  else
    git clone --depth 1 --branch "$BRANCH" "$REPO" "$INSTALL_DIR" \
      || { warn "git clone не удался — пробую tarball"; mkdir -p "$INSTALL_DIR"; \
           curl -fsSL "https://codeload.github.com/exfador/steam-desktop-authenticator-web/tar.gz/${BRANCH}" \
             | tar -xz --strip-components=1 -C "$INSTALL_DIR"; }
  fi
  [ -f "$INSTALL_DIR/deploy.sh" ] || die "Не удалось скачать проект (нет $INSTALL_DIR/deploy.sh). Проверьте REPO/BRANCH."
  log "Запускаю deploy из $INSTALL_DIR"
  cd "$INSTALL_DIR"
...
log "nginx…"
apt_install nginx
[ "$SETUP_AUTH" = "yes" ] && apt_install apache2-utils
